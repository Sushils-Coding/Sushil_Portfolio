...
const SKILLS_DATA = {
  ...
  other: ["Java", "Git", "GitHub", "Postman", "VS Code", "Figma"],
};
...
const cardVariants = {
  ...
  hover: {
    ...
    scale: 1.03,
    boxShadow:
      "0 20px 25px -5px rgba(133, 76, 230, 0.3), 0 10px 10px -5px rgba(133, 76, 230, 0.1)",
    transition: {
...
/**
 * SkillCard - Reusable component for displaying skill categories
 */
const SkillCard = ({ title, skills, glowPosition }) => (
  <motion.div
  ...
  >
    <div
      className={`absolute ${glowPosition} w-32 h-32 rounded-full blur-[60px]`}
    />
    <motion.h2
      ...
      >
        <div className="flex overflow-x-auto pb-8 md:overflow-visible md:flex-wrap md:justify-center gap-6 md:gap-8 scrollbar-hide">
          <SkillCard
            ...
            glowPosition="-top-10 -right-10 bg-purple-500/20"
          />
          <SkillCard
            title="Backend"
            skills={SKILLS_DATA.backend}
            glowPosition="-bottom-10 -left-10 bg-blue-500/20"
          />
          <SkillCard
            title="Others"
            skills={SKILLS_DATA.other}
            glowPosition="-top-10 -left-10 bg-amber-500/20"
          />
        </div>
      </motion.div>
    </motion.div>
